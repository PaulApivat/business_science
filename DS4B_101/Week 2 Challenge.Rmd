...
```{r}
bike_orderlines_tbl2 <- bike_orderlines_tbl %>%
...
bike_orderlines_tbl2$Aluminum <- ifelse(is.na(bike_orderlines_tbl2$Aluminum), 0, bike_orderlines_tbl2$Aluminum)

bike_orderlines_tbl2$Carbon <- ifelse(is.na(bike_orderlines_tbl2$Carbon), 0, bike_orderlines_tbl2$Carbon)

bike_orderlines_tbl2 <- bike_orderlines_tbl2 %>%
...
head(bike_orderlines_tbl2)
    
```

Answer: Not all combinations of primary/secondary bike category contain both Aluminum and Carbon frame.
The following Category_2 models do not contain both: Over Mountain, Triathlon, Cyclocross 
...
bike_orderlines_tbl %>%
    ...
    ungroup() %>%
    spread(key = frame_material, value = Sales) %>%
    # efficient way to find NA and replace with 0
    replace_na(list(Aluminum = 0, Carbon = 0)) %>%
    mutate(total_sales = Aluminum + Carbon) %>%
    arrange(desc(total_sales)) %>%
    # more advanced way to implement scales::dollar
