...
```{r}
# STOCK PRICES
sp_500_prices_tbl <- read_rds("week_6_data/week_6_data/sp_500_prices_tbl.rds")
sp_500_prices_tbl
```
...
```{r}
# SECTOR INFORMATION
sp_500_index_tbl <- read_rds("week_6_data/week_6_data/sp_500_index_tbl.rds")
sp_500_index_tbl
```
...
sp_500_prices_tbl %>% 
    ...
    mutate(adjusted_lag = case_when(
        is.na(adjusted_lag) ~ adjusted,
        TRUE ~ adjusted_lag
    )) %>%
    mutate(
        diff_adjusted = adjusted - adjusted_lag,
        pct_return = diff_adjusted / adjusted_lag
        ) %>%
    select(symbol, date, pct_return)

##

sp_500_daily_returns_tbl_paul <- sp_500_prices_tbl %>%
    select(symbol, date, adjusted) %>%
    # filter 2018 and onwards
    filter((date %>% year()) >= 2018) %>% 
    group_by(symbol) %>%
    # lag, by default
...
glimpse(sp_500_daily_returns_tbl_paul)
...
```{r}
sp_500_daily_returns_tbl <- read_rds("week_6_data/week_6_data/sp_500_daily_returns_tbl.rds")
...
stock_date_matrix_tbl_paul <- sp_500_daily_returns_tbl_paul %>%
...
We're going to import the correct results first (just in case you were not able to complete the last step).
```{r}
stock_date_matrix_tbl <- read_rds("week_6_data/week_6_data/stock_date_matrix_tbl.rds")
...
kmeans_obj_paul <- stock_date_matrix_tbl_paul %>%
    select(-symbol) %>%
...
```{r}
# original kmeans_mapper function
...
# my own function using stock_date_matrix_tbl_paul
kmeans_mapper_paul <- function(center = 3) {
    stock_date_matrix_tbl_paul %>%
        select(-symbol) %>%
        kmeans(centers = center, nstart = 20)
}
...
k_means_mapped_tbl_paul <- tibble(centers = 1:30) %>%
    mutate(k_means = centers %>% map(kmeans_mapper_paul)) %>%
...
We're going to import the correct results first (just in case you were not able to complete the last step).
```{r}
k_means_mapped_tbl <- read_rds("week_6_data/week_6_data/k_means_mapped_tbl.rds")
```
...
umap_results_paul <- stock_date_matrix_tbl_paul %>%
    select(-symbol) %>%
    umap()
...
umap_results_tbl_paul <- umap_results_paul$layout %>% 
    as_tibble() %>%
    bind_cols(
        stock_date_matrix_tbl_paul %>% select(symbol)
    )
...
```{r}
k_means_mapped_tbl <- read_rds("week_6_data/week_6_data/k_means_mapped_tbl.rds")
umap_results_tbl   <- read_rds("week_6_data/week_6_data/umap_results_tbl.rds")
```
...
k_means_obj_paul_version <- k_means_mapped_tbl_paul %>%
    filter(centers == 10) %>%
    pull(k_means) %>%
    pluck(1)
...
umap_kmeans_results_tbl_paul <- k_means_obj_paul_version %>%
    augment(stock_date_matrix_tbl_paul) %>%
    select(symbol, .cluster) %>%
    left_join(umap_results_tbl_paul, by = 'symbol') %>%
    # left_join the result with the result of another operation
